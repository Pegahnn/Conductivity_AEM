...
from NNgraph import GCNReg_add, GCNReg, GATReg_add
...
def objective_with_cv(trial, args, smlstr_train, logCMC_train, results_file):
    # ---- hyperparams ----
    #added by Pegah 1e-4, 1e-1
 #  lr = trial.suggest_float("lr", 1e-4, 5e-3, log=True)
    lr = trial.suggest_float("lr", 1e-5, 5e-3, log=True)
    
    batch_size = trial.suggest_categorical("batch_size", [4, 8, 16])
   #hidden_dim = trial.suggest_categorical("unit_per_layer", [64, 128, 256, 512])
    hidden_dim = trial.suggest_categorical("unit_per_layer", [128, 256, 384])
    weight_decay = trial.suggest_float("weight_decay", 1e-7,1e-4, log=True)
   #dropout = trial.suggest_float("dropout", 0.1, 0.5)
   #dropout = trial.suggest_float("dropout", 0.1, 0.4)
    #added by Pegah 1e-6, 1e-2
   #weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-4, log=True)
   

    # ---- model+collate ----
    if args.gnn_model == "GCNReg_add":
        model_class, collate_fn = GCNReg_add, collate_add
    elif args.gnn_model == "GCNReg":
        model_class, collate_fn = GCNReg, collates
    elif args.gnn_model == "GCNReg":
        model_class, collate_fn = GCNReg, collates
    elif args.gnn_model == "GATReg_add":
        model_class, collate_fn = GATReg_add, collate_add
    else:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(smlstr_train), start=1):
        #print(f"\n=== Trial {trial.number} | Fold {fold}/{args.cv} ===", flush=True)
        y_val_fold = logCMC_train[va_idx]
        print(f"[Trial {trial.number} | Fold {fold+1}] Var(y_val) = {np.var(y_val_fold):.6f}, Mean(y_val) = {np.mean(y_val_fold):.6f}")
        ######33
        ...
        # ---- build model ----
        if args.gnn_model == "GCNReg_add":
            model = model_class(in_dim=args.dim_input, extra_in_dim=args.num_feat,
                                hidden_dim=hidden_dim, n_classes=1, saliency=False)
        else:
            model = model_class(in_dim=args.dim_input, hidden_dim=hidden_dim,
                                n_classes=1, saliency=False)
        ...
        try:
            for epoch in range(args.epochs):
                ...
                val_r2 = r2_score(trues, preds) if preds.size and trues.size else -999.0

                # per-epoch log
                print(f"[T{trial.number} F{fold}] Epoch {epoch:03d} | "
                      f"Val Loss: {mean_val_loss:.6f} | Val R²: {val_r2:.6f}", flush=True)

                # early stopping on loss (stable)
                if mean_val_loss < best_val_loss:
                    ...
                    if patience_counter >= patience:
                        print(f"[T{trial.number} F{fold}] Early stopping at epoch {epoch}", flush=True)
                        break

            cv_r2_scores.append(best_val_r2)

        except Exception as e:
            # surface fold-specific crashes instead of silently failing
            import traceback
    ...
    print(f"\n✅ Trial {trial.number} | Mean CV R² across {args.cv} folds: {mean_r2:.6f}", flush=True)

    # persist trial row
    with open(results_file, "a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([trial.number, mean_r2, lr, batch_size, hidden_dim, weight_decay])

    return mean_r2


def optuna_cv_runner(args, smlstr, logCMC, smlstr_test, logCMC_test):
    os.makedirs(args.save_dir, exist_ok=True)
    results_file = os.path.join(args.save_dir, "trials_results.csv")
    if not os.path.exists(results_file):
        with open(results_file, "w", newline="") as f:
            csv.writer(f).writerow(["trial_number", "cv_r2", "lr", "batch_size", "unit_per_layer", "weight_decay"])

    study = optuna.create_study(direction="maximize")
    study.optimize(lambda tr: objective_with_cv(tr, args, smlstr, logCMC, results_file),
    ...
    batch_size, hidden_dim = best["batch_size"], best["unit_per_layer"]
    lr, weight_decay = best["lr"], best["weight_decay"]

    if args.gnn_model == "GCNReg_add":
        model, collate_fn = GCNReg_add(args.dim_input, args.num_feat, hidden_dim, 1, False), collate_add
    else:
        model, collate_fn = GCNReg(args.dim_input, hidden_dim, 1, False), collates
...
if __name__ == "__main__":
    ...
    optuna_cv_runner(args, smlstr_train, logCMC_train, smlstr_test, logCMC_test)
